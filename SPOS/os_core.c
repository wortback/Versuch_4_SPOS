...
//! A variable to store the beginning address of the heap (set by linker)
...
void os_init(void) {
	...
	// Check if the first address of the heap is not too close to globals
...
void os_errorPStr(char const* str) {
	...
	bool interr_deactivated = false;
	
	// Check if the 7th bit of SREG is set
	if((SREG & 0b10000000) == 0b10000000){
		interr_deactivated = true;
	}
		
	// reset the 7th bit to turn off the interrupt
	SREG &= (0<<7);
	lcd_clear();
	...
	// Wait for ESC and ENTER pressed
	while(os_getInput()!=0b00001001){}
		
	// Wait for them to be released
	os_waitForNoInput();
	
	// Restore the 7th bit of SREG
	if(interr_deactivated == true){
		SREG |= (1<<7);
		interr_deactivated = false;
	}
